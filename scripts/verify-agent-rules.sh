...
project_agent_readme="$repo_root/Project and Agent/README.md"
parent_agents="$repo_root/Project and Agent/Parent Project Set/AGENTS.md"
parent_independent_agents="$repo_root/Project and Agent/Parent Independent Projects/AGENTS.md"
...
readme_assets="$repo_root/docs/assets/readme"
...
require_file "$project_agent_readme" "project and agent README exists"
require_file "$parent_agents" "parent project set AGENTS exists"
require_file "$parent_independent_agents" "parent independent projects AGENTS exists"
...
require_file "$repo_root/tests/cases/astra-boundaries.md" "Astra behavior acceptance cases exist"
require_file "$repo_root/skill-updates/README.md" "Astra skill installation instructions exist"
require_file "$repo_root/skill-updates/superpowers/astra.patch" "Superpowers update patch exists"
require_file "$repo_root/skill-updates/superpowers/LICENSE" "Superpowers redistribution license exists"
require_file "$repo_root/skill-updates/Guofu-improving-Agent/references/record-formats.md" "Learning format route target exists"
...
require_file "$repo_root/tests/cases/independent-project-new-project.md" "independent project new project case exists"
...
require_file "$readme_assets/01-rules-chaos.png" "README rules chaos visual exists"
...
require_text "$parent_independent_agents" "/Users/guofu/Documents/独立项目" "parent independent projects records directory scope"
require_text "$parent_independent_agents" "/Users/guofu/.codex/agent-templates/project-agent/" "parent independent projects records template source"
require_text "$parent_independent_agents" "本文件是可选父级模板，不是必备文件" "parent independent projects declares optional status"
require_text "$parent_independent_agents" "独立项目列表" "parent independent projects avoids default project-list install"
require_text "$parent_independent_agents" '如果新项目目录已经存在但缺少 `AGENTS.md`' "parent independent projects catches empty formal projects"
require_text "$parent_independent_agents" "不是一次性临时对话" "parent independent projects distinguishes formal projects"
require_text "$project_agent_readme" "Parent Independent Projects" "project and agent README documents independent parent template"
require_text "$project_agent_readme" "可选父级模板样例" "project and agent README marks parent template optional"
...
require_text "$repo_root/tests/cases/independent-project-new-project.md" "空目录" "independent project case describes empty project initialization"
require_text "$repo_root/tests/cases/independent-project-new-project.md" "trusted project 不等于项目初始化完成" "independent project case guards trusted project gap"
...
require_text "$root_readme" "已针对 GPT-6 Astra 的提示词与技能行为做适配" "root README states GPT-6 Astra adaptation"
require_text "$root_readme" "已优化 Superpowers / Skill 的触发逻辑" "root README states Superpowers skill trigger optimization"
require_text "$root_readme" "git clone --branch v0.3.0" "root README uses current release tag"
require_text "$root_readme" "Parent Independent Projects" "root README documents independent parent template"
require_text "$root_readme" "父级目录模板是可选工具，不是默认安装项" "root README states parent templates are optional"
require_text "$root_readme" "docs/assets/readme/01-rules-chaos.png" "root README references rules chaos visual"
require_text "$root_readme" "docs/assets/readme/02-learning-capture.png" "root README references learning capture visual"
require_text "$root_readme" "docs/assets/readme/03-agent-architecture.png" "root README references agent architecture visual"
require_no_text "$root_readme" "docs/assets/readme/04-gpt56-adaptation.png" "root README does not display historical GPT-5.6 visual"
require_text "$root_readme" "docs/assets/readme/05-skill-release-loop.png" "root README references skill release loop visual"
require_text "$wiki_root/Home.md" "v0.3.0" "wiki Home records current release"
require_text "$wiki_root/Home.md" "已优化 Superpowers / Skill 的触发逻辑" "wiki Home states Superpowers skill trigger optimization"
require_text "$wiki_root/Installation-Guide.md" "git clone --branch v0.3.0" "wiki installation uses release tag"
require_text "$wiki_root/Installation-Guide.md" "Parent Independent Projects" "wiki installation documents independent parent template"
require_text "$wiki_root/Installation-Guide.md" "默认跳过" "wiki installation marks parent templates optional"
require_text "$wiki_root/Version-History.md" "v0.3.0 - Astra 指令适配" "wiki version history records v0.3.0"
