...
- 涉及架构、数据、权限、核心流程、重要体验路径或具有实质依赖的多文件改动；机械替换不自动触发完整流程。
...
沿用全局按行为风险验证的要求；保留已有工作，补足有效测试，必要检查通过后不无故扩大或重复验证。
...
- 跨项目规则：先提示用户确认，再考虑更新全局 `/Users/guofu/.codex/AGENTS.md`。
